...
def docker_api_network(req_container_id):
    try:
        response = requests.post(f'http://container_backend:{str(int(os.getenv("CONTAINER_PORT"))+1)}/dockerrest', json={"req_method":"network","req_container_id":req_container_id})
        res_json = response.json()
...
with gr.Blocks() as app:
    gr.Markdown(
        """
        # Welcome!
        Load Hugging Face models on different ports
    ...
    dl_timer = gr.Timer(2,active=False)
    dl_timer.tick(check_rx_change, inputs=[download_status], outputs=[create_response])
    ...
    def render_container(render_container_list):
        docker_container_list = get_docker_container_list()
        docker_container_list_running = [task for task in docker_container_list if task["State"]["Status"] == "running"]
        docker_container_list_not_running = [task for task in docker_container_list if task["State"]["Status"] != "running"]
    ...
    def check_container_running(loaded_model):
        try:
            print(f'check if container loaded?')
            docker_container_list = get_docker_container_list()
            docker_container_list_running = [d for d in docker_container_list if d["name"] == loaded_model]
            print(f'docker_container_list_running {docker_container_list_running}')
    ...
    timer.tick(docker_api_network, create_response, download_status)
    
    dl_timer2 = gr.Timer(1,active=False)
    dl_timer2.tick(check_rx_change, inputs=[download_status], outputs=[create_response])
    
    btn_dl.click(lambda: gr.Timer(active=True), None, timer).then(lambda: gr.Timer(active=True), None, dl_timer).then(lambda: gr.update(label="Download Progress:",visible=True), None, create_response).then(docker_api_create,inputs=[model_dropdown,selected_model_pipeline_tag,port_model,port_vllm],outputs=create_response).then(refresh_container_list, outputs=[container_state]).then(lambda: gr.Timer(active=True), None, dl_timer2)
